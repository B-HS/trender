...
# cron이 spawn하는 자식 프로세스는 컨테이너 env를 못 받으므로 파일로 dump해 둔다.
{
  for var in DATABASE_URL TZ LOG_LEVEL LLM_PROVIDERS \
             OLLAMA_CLOUD_KEY OLLAMA_CLOUD_HOST OLLAMA_CLOUD_MODEL OLLAMA_CLOUD_MODEL_LIGHT OLLAMA_CLOUD_TIMEOUT_SECONDS \
             OLLAMA_HOST OLLAMA_LOCAL_MODEL OLLAMA_LOCAL_MODEL_LIGHT OLLAMA_NUM_CTX OLLAMA_TIMEOUT_SECONDS \
             OMLX_HOST OMLX_MODEL OMLX_MODEL_LIGHT \
             OPENROUTER_API_KEY OPENROUTER_MODEL OPENROUTER_MODEL_LIGHT OPENROUTER_HOST OPENROUTER_APP_TITLE OPENROUTER_REFERER \
             OPENAI_OAUTH_MODEL OPENAI_OAUTH_MODEL_LIGHT OPENAI_OAUTH_BASE_URL OPENAI_OAUTH_TOKEN OPENAI_OAUTH_AUTH_FILE \
