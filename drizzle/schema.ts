import { mysqlTable, mysqlSchema, AnyMySqlColumn, index, foreignKey, primaryKey, unique, bigint, varchar, mysqlEnum, mediumtext, datetime, timestamp, int, date } from "drizzle-orm/mysql-core"
...
export const articles = mysqlTable("articles", {
	...
	lang: mysqlEnum(['ko','ja','en']).notNull(),
	...
	fetchedAt: timestamp("fetched_at", { mode: 'string' }).default(sql`(now())`).notNull(),
...
export const keywordsExtracted = mysqlTable("keywords_extracted", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const reportItems = mysqlTable("report_items", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	reportId: bigint("report_id", { mode: "number" }).notNull().references(() => reports.id, { onDelete: "cascade" } ),
	articleId: bigint("article_id", { mode: "number" }).notNull().references(() => articles.id),
	rank: int().notNull(),
},
(table) => [
...
export const reports = mysqlTable("reports", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	kind: mysqlEnum(['daily','weekly']).notNull(),
	lang: mysqlEnum(['ko','ja','en']).notNull(),
...
export const sourceStats = mysqlTable("source_stats", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	sourceId: bigint("source_id", { mode: "number" }).notNull().references(() => sources.id, { onDelete: "cascade" } ),
	// you can use { mode: 'date' }, if you want to have Date as type for this column
